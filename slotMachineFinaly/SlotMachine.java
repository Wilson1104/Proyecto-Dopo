...
public class SlotMachine
{
    ...
    /**
     * Crea una maquina con n ruedas y n simbolos de colores diferentes.
     * Todas las ruedas tienen los mismos simbolos en el mismo orden y
     * cada una queda mostrando un simbolo elegido al azar, sin que la
     * configuracion inicial sea ganadora. La maquina queda invisible.
     *
     * @param n el numero de ruedas y de simbolos (debe ser mayor a 1).
     */
    public SlotMachine(int n)
    {
        this();
        if (n < 2) {
            fail("La maquina necesita al menos dos ruedas.");
            return;
        }
        String[] colors = colorsFor(n);
        for (int w = 1; w <= n; w++) {
            addWheel(w);
    ...
    public void swap(int wheel1, int wheel2)
    {
        ...
        if (w1.isLocked() || w2.isLocked()) {
            fail("No se puede intercambiar una rueda bloqueada.");
            return;
        }
        wheels.set(p1 - 1, w2);
        wheels.set(p2 - 1, w1);
    ...
    /**
     * Si la maquina esta visible y la configuracion actual es
     * ganadora, muestra un mensaje avisandolo.
     */
    ...
    /**
     * Genera n colores diferentes en formato hexadecimal (#rrggbb),
     * repartiendo el tono uniformemente en la rueda de color.
     *
     * @param n cuantos colores generar.
     * @return un arreglo con n colores diferentes.
     */
    private String[] colorsFor(int n)
    {
        String[] colors = new String[n];
        for (int i = 0; i < n; i++) {
            Color c = Color.getHSBColor((float) i / n, 0.9f, 0.9f);
    ...
    /**
     * Retorna cuantos simbolos diferentes se estan mostrando en este
     * momento en las ruedas de la maquina (los de la configuracion).
     * Es la unica informacion que entrega la testing tool de la maraton.
     *
     * @return el numero de simbolos distintos que se ven.
     */
